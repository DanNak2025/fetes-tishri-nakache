...
export default function Home() {
  ...
  return <main>
    ...
    <section className="summary" aria-labelledby="summary-title"><div><h2 id="summary-title">Récapitulatif</h2><p>Les réponses manquantes apparaissent ici.</p></div><div className="summary-table"><div className="summary-head"><span>Personne</span><span>Présent</span><span>Pas encore répondu</span></div>{planning.names.map((name, index) => { const count = events.filter((event) => planning.attendance[`${event.id}-${index}`] === "yes").length; const missing = events.filter((event) => !planning.attendance[`${event.id}-${index}`]).length; return <div className="summary-row" key={index}><span>{name.trim() || `Personne ${index + 1}`}</span><strong>{count} / {events.length}</strong><em>{missing ? `${missing} en attente` : "Tout répondu"}</em></div>; })}</div></section>
    <section className="planner" aria-labelledby="planner-title"><div className="section-title"><div><h2 id="planner-title">Le planning</h2><p>Chaque personne choisit sa réponse. Les changements sont visibles par tous.</p></div><span className="answer-count">{answeredCount} / {totalAnswers} réponses</span></div><div className="legend"><span><b className="yes">✓</b> Je viens</span><span><b className="maybe">?</b> Pas sûr</span><span><b className="no">–</b> Je ne viens pas</span></div><div className="event-list">{events.map((event) => { const confirmed = planning.names.filter((_, index) => planning.attendance[`${event.id}-${index}`] === "yes").length; return <article className="event" key={event.id}><header><div className="calendar"><strong>{event.date.split(" ")[0].slice(0, 3)}</strong><span>{event.date.match(/\d+/)?.[0]}</span></div><div><h3>{event.name}</h3><p>{event.date}</p></div><small>{confirmed} présent{confirmed > 1 ? "s" : ""}</small></header><div className="responses">{planning.names.map((name, personIndex) => { const current = planning.attendance[`${event.id}-${personIndex}`]; return <div className="response" key={personIndex}><span className="person-name">{name.trim() || `Personne ${personIndex + 1}`}</span><div className="choices" aria-label={`Présence de ${name} pour ${event.name}`}>{statuses.map((status) => <button key={status.key} title={status.label} aria-label={status.label} className={`${status.key} ${current === status.key ? "selected" : ""}`} onClick={() => setStatus(event.id, personIndex, status.key)}>{status.icon}</button>)}</div></div>; })}</div></article>; })}</div></section>
